...
> 💡 不知道是否已安装？打开终端输入 `node -v`，如果显示版本号（如 `v20.0.0`）就说明已安装。

### 第 2 步：安装项目依赖

**方式 A：使用启动脚本（推荐，最简单）**

直接跳到第 3 步，启动脚本会自动安装依赖和初始化数据库。

**方式 B：手动安装（适合熟悉命令行的用户）**
...
> 💡 **重要提示**：
> - 首次运行启动脚本时，会自动安装依赖和初始化数据库，可能需要 **1-5 分钟**，请耐心等待
> - 启动后请**保持启动窗口打开**，关闭应用时在窗口按 `Ctrl + C`（Mac 用户可能需要按 `Control + C`）
> - 等待启动窗口显示 `ready in xxx ms` 和 `Local: http://localhost:3000/` 后，在浏览器打开 `http://localhost:3000` 即可使用！
...
| **Node.js** | v18.0.0 或更高版本（推荐 LTS 版本） |
| **npm** | 随 Node.js 自动安装（通常 v9.0.0+） |
| **磁盘空间** | 至少 500MB 可用空间（用于安装依赖） |
| **内存** | 建议 4GB 或更多 |
| **网络** | 需要网络连接（用于获取股票行情和安装依赖） |

> 💡 **提示**：
> - 如果您的 Node.js 版本低于 v18，请访问 https://nodejs.org/zh-cn 下载最新 LTS 版本
> - 检查 Node.js 版本：在终端运行 `node -v`
> - 检查 npm 版本：在终端运行 `npm -v`
...
```
数据库初始化完成!
数据库路径: /path/to/项目/data/portfolio-guard.db
```

> 💡 **提示**：
> - 这个步骤只需要运行一次
> - 如果以后重新安装，也需要运行这个命令
> - 如果使用启动脚本（`启动应用.sh` 或 `启动应用.bat`），这一步会自动完成，无需手动运行
...
> - ✅ 应用使用端口 **3000**（前端）和 **3001**（后端），如果这些端口被占用，请参考[常见问题 Q4](#q4-端口被占用显示-eaddrinuse)
...
> ⚠️ **注意**：使用命令行启动前，请确保已完成[安装步骤](#-安装步骤)（安装依赖和初始化数据库）。
...
这个命令会同时启动后端（端口 3001）和前端（端口 3000）服务。

**成功标志**：
- 看到 `服务器已启动: http://127.0.0.1:3001`
- 看到 `ready in xxx ms` 和 `Local: http://localhost:3000/`

然后在浏览器打开 `http://localhost:3000` 即可使用。

> ⚠️ **注意**：这个终端窗口需要保持打开，关闭应用时按 `Ctrl + C`。
...
> 💡 **提示**：
> - 关闭应用时，需要分别在这两个终端窗口按 `Ctrl + C`
> - 如果端口被占用，请参考[常见问题 Q4](#q4-端口被占用显示-eaddrinuse)
...
- [ ] ✅ 已安装 Node.js v18+（运行 `node -v` 有版本号显示，如 `v20.0.0`）
- [ ] ✅ 已运行 `npm install` 且没有错误（或使用启动脚本自动安装）
- [ ] ✅ 已运行 `npm run db:init` 且显示"数据库初始化完成"（或使用启动脚本自动初始化）
- [ ] ✅ 已找到启动脚本文件（`启动应用.sh` 或 `启动应用.bat`）
- [ ] ✅ 端口 3000 和 3001 未被其他程序占用
...
1. **数据安全**：您的所有数据保存在项目目录下的 `data/portfolio-guard.db` 文件中，请定期备份
2. **保持窗口打开**：使用启动脚本或命令行启动时，请保持启动窗口打开，关闭应用时按 `Ctrl + C`（Mac 用户可能需要按 `Control + C`）
3. **浏览器地址**：应用启动后，在浏览器打开 `http://localhost:3000`（注意是 **http** 不是 https，端口是 3000）
4. **首次启动**：首次使用启动脚本时，会自动安装依赖和初始化数据库，可能需要等待 **1-5 分钟**，请耐心等待
5. **端口占用**：应用使用端口 3000（前端）和 3001（后端），如果这些端口被占用，请参考[常见问题 Q4](#q4-端口被占用显示-eaddrinuse)
